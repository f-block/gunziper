...
public class diff_match_patch {
    ...
    public static class Patch {
        ...
        public String toString() {
            ...
            return diff_match_patch.unescapeForEncodeUriCompatability(text
                    .toString());
        }
    }
    ...
    private LinkedList<Diff> diff_bisectSplit(String text1, String text2,
        ...
        LinkedList<Diff> diffs = this
                .diff_main(text1a, text2a, false, deadline);
        LinkedList<Diff> diffsb = this.diff_main(text1b, text2b, false,
                deadline);
        ...
        while (thisDiff != null) {
            ...
            case EQUAL:
                if (count_delete + count_insert > 1) {
                    boolean both_types = (count_delete != 0)
                            && (count_insert != 0);
                    ...
                    if (both_types) {
                        ...
                        commonlength = this.diff_commonPrefix(text_insert,
                                text_delete);
                        if (commonlength != 0) {
                        ...
                        commonlength = this.diff_commonSuffix(text_insert,
                                text_delete);
                        if (commonlength != 0) {
                            ...
                            thisDiff.text = text_insert.substring(text_insert
                                    ...
                                    - commonlength)
                                    + thisDiff.text;
                            text_insert = text_insert.substring(0, text_insert
                                    .length()
                                    - commonlength);
                            text_delete = text_delete.substring(0, text_delete
                                    .length()
                                    - commonlength);
                            pointer.previous();
                        }
            ...
            if ((prevDiff.operation == Operation.DELETE)
                ...
                if (overlap_length1 >= overlap_length2) {
                    ...
                    if ((overlap_length2 >= deletion.length() / 2.0)
                        ...
                        prevDiff.text = insertion.substring(0, insertion
                                .length()
                                - overlap_length2);
    ...
    private LinkedList<Diff> diff_compute(String text1, String text2,
        ...
        if (hm != null) {
            ...
            LinkedList<Diff> diffs_a = this.diff_main(text1_a, text2_a,
                    checklines, deadline);
            LinkedList<Diff> diffs_b = this.diff_main(text1_b, text2_b,
                    checklines, deadline);
            // Merge the results.
            diffs = diffs_a;
    ...
    protected String[] diff_halfMatch(String text1, String text2) {
        ...
        String[] hm1 = this.diff_halfMatchI(longtext, shorttext, (longtext
                .length() + 3) / 4);
        // Check again based on the third quarter.
        String[] hm2 = this.diff_halfMatchI(longtext, shorttext, (longtext
                .length() + 1) / 2);
        String[] hm;
        ...
        while (thisDiff != null) {
            ...
            case EQUAL:
                ...
                if ((count_delete >= 1) && (count_insert >= 1)) {
                    ...
                    for (Diff newDiff:this.diff_main(text_delete, text_insert,
                            false, deadline)) {
                        pointer.add(newDiff);
                    }
                }
    ...
    protected int match_bitap(String text, String pattern, int loc) {
        ...
        if (best_loc != -1) {
            ...
            if (best_loc != -1) {
                score_threshold = Math.min(this.match_bitapScore(0, best_loc,
                        loc, pattern), score_threshold);
            }
        }
        ...
        for (int d = 0; d < pattern.length(); d++) {
            ...
            for (int j = finish; j >= start; j--) {
                ...
                if ((rd[j] & matchmask) != 0) {
                    double score = this
                            .match_bitapScore(d, j - 1, loc, pattern);
    ...
    protected void patch_addContext(Patch patch, String text) {
        ...
        while ((text.indexOf(pattern) != text.lastIndexOf(pattern))
            ...
            pattern = text
                    .substring(Math.max(0, patch.start2 - padding), Math.min(
                            text.length(), patch.start2 + patch.length1
                                    + padding));
        }
        ...
        // Add the suffix.
        String suffix = text.substring(patch.start2 + patch.length1, Math.min(
                text.length(), patch.start2 + patch.length1 + padding));
        if (suffix.length() != 0) {
    ...
    public Object[] patch_apply(LinkedList<Patch> patches, String text) {
        ...
        for (Patch aPatch:patches) {
            ...
            if (text1.length() > this.Match_MaxBits) {
                ...
                // case of
                // a monster delete.
                start_loc = this.match_main(text, text1.substring(0,
                        this.Match_MaxBits), expected_loc);
                if (start_loc != -1) {
                    end_loc = this.match_main(text, text1.substring(text1
                            .length()
                            - this.Match_MaxBits), expected_loc
            ...
            if (start_loc == -1) {
                ...
                String text2;
                if (end_loc == -1) {
                    text2 = text.substring(start_loc, Math.min(start_loc
                            + text1.length(), text.length()));
                }
                else {
                    text2 = text.substring(start_loc, Math.min(end_loc
                            + this.Match_MaxBits, text.length()));
                }
                if (text1.equals(text2)) {
                    // Perfect match, just shove the replacement text in.
                    text = text.substring(0, start_loc)
                            + this.diff_text2(aPatch.diffs)
            ...
            while (!bigpatch.diffs.isEmpty()) {
                ...
                while (!bigpatch.diffs.isEmpty()
                    ...
                    if (diff_type == Operation.INSERT) {
                        ...
                        // stomach.
                        diff_text = diff_text.substring(0, Math.min(diff_text
                                .length(), patch_size - patch.length1
                                - this.Patch_Margin));
                        patch.length1 += diff_text.length();
                        start1 += diff_text.length();
                ...
                precontext = precontext.substring(Math.max(0, precontext
                        .length()
                        - this.Patch_Margin));
                // Append the end context for this patch.
                if (this.diff_text1(bigpatch.diffs).length() > this.Patch_Margin) {
                    postcontext = this.diff_text1(bigpatch.diffs).substring(0,
                            this.Patch_Margin);
                }
                else {
