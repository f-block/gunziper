...
import difflib.diffutils.DiffRow.Tag;


/**
 * This class for generating DiffRows for side-by-sidy view.
 * You can customize the way of generating. For example, show inline diffs on
 * not, ignoring
 * white spaces or/and blank lines and so on. All parameters for generating are
 * optional. If you do
...
public class DiffRowGenerator {
    ...
    public static class Builder {
         ...
         * 
         * @param cssClass
         *            the tag to set. Without any quotes, just word. Default:
         *            editNewInline.
         * @return builder with configured ignoreBlankLines parameter
         */
     ...
     */
    private static <T> String join(final Iterable<T> objs,
     ...
     * 
     * @param startPosition
     *            the position from which tag should start. The counting start
     *            from a zero.
     * @param endPosition
     *            the position before which tag should should be closed.
     * @param tag
     *            the tag name without angle brackets, just a word
     * @param cssClass
     *            the optional css class
     */
    @SuppressWarnings("unchecked")
    public static LinkedList<String> wrapInTag(LinkedList<String> sequence,
            int startPosition, int endPosition, String tag, String cssClass) {
    ...
    private void addInlineDiffs(Delta delta) {
        ...
        for (Character character:DiffRowGenerator.join(orig, "\n")
                .toCharArray()) {
            origList.add(character.toString());
        }
        LinkedList<String> revList = new LinkedList<String>();
        for (Character character:DiffRowGenerator.join(rev, "\n").toCharArray()) {
            revList.add(character.toString());
        }
        List<Delta> inlineDeltas = DiffUtils.diff(origList, revList)
                .getDeltas();
        if (inlineDeltas.size() < 3) {
            ...
            for (Delta inlineDelta:inlineDeltas) {
                ...
                if (inlineDelta.getClass().equals(DeleteDelta.class)) {
                    origList = DiffRowGenerator.wrapInTag(origList, inlineOrig
                            .getPosition(), inlineOrig.getPosition()
                            + inlineOrig.size() + 1, this.InlineOldTag,
                            this.InlineOldCssClass);
                }
                else if (inlineDelta.getClass().equals(InsertDelta.class)) {
                    revList = DiffRowGenerator.wrapInTag(revList, inlineRev
                            .getPosition(), inlineRev.getPosition()
                            + inlineRev.size() + 1, this.InlineNewTag,
                            this.InlineNewCssClass);
                }
            ...
            delta.getOriginal().setLines(
                    Arrays.asList(origResult.toString().split("\n")));
            delta.getRevised().setLines(
    ...
    public List<DiffRow> generateDiffRows(List<String> original,
        ...
        return this.generateDiffRows(original, revised, DiffUtils.diff(
                original, revised));
    }

    /**
     * Generates the DiffRows describing the difference between original and
     * revised texts using the
     * given patch. Useful for displaying side-by-side diff.
     * 
     * @param original
     *            the original text
    ...
    @SuppressWarnings("unchecked")
    public List<DiffRow> generateDiffRows(List<String> original,
            List<String> revised, Patch patch) {
        ...
        for (int i = 0; i < deltaList.size(); i++) {
            ...
            orig.setLines(StringUtills
                    .normalize((List<String>) orig.getLines()));
            rev.setLines(StringUtills.normalize((List<String>) rev.getLines()));

            orig.setLines(StringUtills.wrapText((List<String>) orig.getLines(),
                    this.columnWidth));
            rev.setLines(StringUtills.wrapText((List<String>) rev.getLines(),
                    this.columnWidth));
            ...
            if (orig.size() == rev.size()) {
                for (int j = 0; j < orig.size(); j++) {
                    diffRows
                            .add(new DiffRow(Tag.CHANGE, (String) orig
                                    .getLines().get(j), (String) rev.getLines()
                                    .get(j)));
                }
            }
            else if (orig.size() > rev.size()) {
                for (int j = 0; j < orig.size(); j++) {
                    diffRows.add(new DiffRow(Tag.CHANGE, (String) orig
                            .getLines().get(j),
                            rev.getLines().size() > j ? (String) rev.getLines()
                                    .get(j):""));
                }
            }
            else {
                for (int j = 0; j < rev.size(); j++) {
                    diffRows.add(new DiffRow(Tag.CHANGE,
                            orig.getLines().size() > j ? (String) orig
                                    .getLines().get(j):"", (String) rev
                                    .getLines().get(j)));
                }
            }
            endPos = orig.last() + 1;
