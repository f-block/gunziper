...
import difflib.diffutils.myers.MyersDiff;
...
public class DiffUtils {
    ...
    /**
     * Compute the difference between the original and revised texts with
    ...
    public static Patch diff(List<?> original, List<?> revised) {
        ...
        return DiffUtils
                .diff(original, revised, DiffUtils.defaultDiffAlgorithm);
    }

    /**
     * Compute the difference between the original and revised texts with given
     * diff algorithm
     * 
     * @param original
     *            the original text
    ...
    public static List<String> generateUnifiedDiff(String original,
            String revised, List<String> originalLines, Patch patch,
            int contextSize) {
        ...
        if (!patch.getDeltas().isEmpty()) {
            ...
            List<Delta> deltas = new ArrayList<Delta>(); // current list of
                                                         // Delta's to process
            ...
            if (patchDeltas.size() > 1) {
                for (int i = 1; i < patchDeltas.size(); i++) {
                    ...
                    if ((position + delta.getOriginal().size() + contextSize) >= (nextDelta
                        ...
                        // it.
                        List<String> curBlock = DiffUtils.processDeltas(
                                originalLines, deltas, contextSize);
                        ret.addAll(curBlock);
                        deltas.clear();
                        deltas.add(nextDelta);
                    }
            ...
            List<String> curBlock = DiffUtils.processDeltas(originalLines,
                    deltas, contextSize);
            ret.addAll(curBlock);
            return ret;
        }
    ...
    public static Patch parseUnifiedDiff(List<String> diff) {
        ...
        for (String line:diff) {
            ...
            Matcher m = DiffUtils.unifiedDiffChunkRe.matcher(line);
            if (m.find()) {
        ...
        if (rawChunk.size() != 0) {
            ...
            patch.addDelta(new ChangeDelta(
                    new Chunk(old_ln - 1, oldChunkLines), new Chunk(new_ln - 1,
                            newChunkLines)));
            rawChunk.clear();
        }
    ...
    private static List<String> processDeltas(List<String> origLines,
            List<Delta> deltas, int contextSize) {
        ...
        while (deltaIndex < deltas.size()) { // for each of the other Deltas
            ...
            buffer.addAll(DiffUtils.getDeltaText(nextDelta)); // output the
                                                              // Delta
