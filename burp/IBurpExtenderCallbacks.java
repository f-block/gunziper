...
public interface IBurpExtenderCallbacks {
     ...
     *            The <code>IHttpRequestResponse</code> object
     *            to which the markers should be applied.
     * @param requestMarkers
     *            A list of index pairs representing the offsets of
     *            markers to be applied to the request message. Each item in the
     *            list must
     *            be an int[2] array containing the start and end offsets for
     *            the marker.
     *            The markers in the list should be in sequence and not
     *            overlapping. This
     *            parameter is optional and may be <code>null</code> if no
     *            request markers
     *            are required.
     * @param responseMarkers
     *            A list of index pairs representing the offsets of
     *            markers to be applied to the response message. Each item in
     *            the list must
     *            be an int[2] array containing the start and end offsets for
     *            the marker.
     *            The markers in the list should be in sequence and not
     ...
     * @param controller
     *            An object created by the extension that implements the
     *            <code>IMessageEditorController</code> interface. This
     *            parameter is
     *            optional and may be <code>null</code>. If it is provided, then
     *            the
     *            message editor will query the controller when required to
     *            obtain details
     *            about the currently displayed message, including the
     *            <code>IHttpService</code> for the message, and the associated
     *            request or
     *            response message. If a controller is not provided, then the
     *            message
     ...
     */
    IScanQueueItem doActiveScan(String host, int port, boolean useHttps,
            byte[] request);
     ...
     *            positions of the insertion points that should be scanned. Each
     *            item in
     ...
     */
    IScanQueueItem doActiveScan(String host, int port, boolean useHttps,
            byte[] request, List<int[]> insertionPointOffsets);
     ...
     */
    void doPassiveScan(String host, int port, boolean useHttps, byte[] request,
            byte[] response);
     ...
     * which can be used by the extension
     ...
     *            order to extract a specific subset of scan issues. The method
     *            performs a
     *            simple case-sensitive text match, returning all scan issues
     *            whose URL
     ...
     * @param urlPrefix
     *            This parameter can be used to specify a URL prefix, in
     *            order to extract a specific subset of the site map. The method
     *            performs a
     *            simple case-sensitive text match, returning all site map items
     *            whose URL
     *            begins with the specified prefix. If this parameter is null,
     *            the entire
     ...
     * 
     * @param toolFlag
     *            A flag identifying a Burp tool ( <code>TOOL_PROXY</code>,
     *            <code>TOOL_SCANNER</code>, etc.). Tool flags are defined
     *            within this
     *            interface.
     ...
     * 
     * @param httpService
     *            The HTTP service to which the request should be sent.
     * @param request
     *            The full HTTP request.
     ...
     */
    byte[] makeHttpRequest(String host, int port, boolean useHttps,
            byte[] request);
     ...
     * @param httpRequestResponse
     *            The <code>IHttpRequestResponse</code> object
     *            whose request and response messages are to be saved to
     *            temporary files.
     * @return An object that implements the
     ...
     *            then any existing setting with the specified name will be
     *            removed.
     */
     ...
     * @param request
     *            The full HTTP request.
     * @param payloadPositionOffsets
     *            A list of index pairs representing the
     *            payload positions to be used. Each item in the list must be an
     *            int[2]
     *            array containing the start and end offsets for the payload
     *            position.
     */
    void sendToIntruder(String host, int port, boolean useHttps,
            byte[] request, List<int[]> payloadPositionOffsets);
     ...
     *            tab containing the request. If this value is <code>null</code>
     *            then a
     *            default tab index will be displayed.
     */
    void sendToRepeater(String host, int port, boolean useHttps,
            byte[] request, String tabCaption);
     ...
     * 
     * @param cookie
     *            An <code>ICookie</code> object containing details of the
     *            cookie to be updated. If the cookie jar already contains a
     *            cookie that
     *            matches the specified domain and name, then that cookie will
     *            be updated
     *            with the new value and expiration, unless the new value is
     *            <code>null</code>, in which case the cookie will be removed.
     *            If the
     *            cookie jar does not already contain a cookie that matches the
     *            specified
